...
{
    /// <summary>
    /// Pulls a name out of a message prefix, by convention <c>[Tag] the rest of the message</c>. The
    /// pattern is user-configurable; an invalid one falls back to <paramref name="fallbackPattern"/> and
    /// reports itself through <see cref="IsValid"/> so the settings page can say so. Results are cached by
    /// message prefix, which makes repeated extraction over a full buffer cheap.
    /// </summary>
    internal abstract class PrefixExtractor
    {
        ...
        protected PrefixExtractor(string pattern, string fallbackPattern)
        {
            ...
            {
                _regex = new Regex(fallbackPattern, RegexOptions.CultureInvariant, MatchTimeout);
                IsValid = false;
            }
        }
