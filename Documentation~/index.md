...
Ctrl+click adds a row to the selection, Shift+click takes a range, and Ctrl+A selects everything shown. Right-clicking inside a selection offers to copy all of it: the messages, the messages with their stacks, or the whole run as a bug report carrying the Unity and package versions. Ctrl+C copies the selection as plain messages. The single-row actions stay in the same menu and act on the row under the pointer, and the status bar counts the selection. It survives entries streaming in: the window tracks the entries, not the row numbers, so a filter change or eviction keeps whatever is still on screen selected.
...
File > Theme in the toolbar switches the window's look. Native, the default, takes every colour from the Editor's own theme variables, so it matches the light or dark skin and any change Unity makes to it. Obsidian is a dark theme with a blue accent, Paper a warm light one, Sci-fi a deep navy console with a cyan accent, a faint scanline overlay and a glowing frame around the source preview, and Ember a warm black console with an amber accent whose selected row glows from its left edge. The designed themes set messages, stack frames and source lines in JetBrains Mono, bundled in `Editor/UI/Fonts` under the SIL Open Font License; Native keeps the Editor font. The choice is a per-user preference, not a project setting.
